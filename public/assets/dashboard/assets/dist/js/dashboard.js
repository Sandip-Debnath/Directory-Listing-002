(function ($) {
    ...
    */
    ...
    /*-------------------------------------------
        File Up
    --------------------------------------------- */
    if (typeof window !== 'undefined' && $('.fileUp').length && $.fn && $.fn.FancyFileUpload) {
        $('.fileUp').FancyFileUpload({
          params: { action: 'fileuploader' },
          maxfilesize: 1000000
        });
      } else {
        // Optional: silent fallback or console hint
        console.warn('FancyFileUpload plugin missing or .fileUp not found');
      }
